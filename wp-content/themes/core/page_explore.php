...
function section_do_title(){
	?> <div class="tab-title"><h1><a href="<?php echo get_bloginfo('url'); ?>/explore-apu/">Explore APU</a></h1></div><?php
}

//Add in the sidebar menu
add_action('genesis_sidebar', 'template_do_sidebar_menu', 2);
function template_do_sidebar_menu(){
	$menu_name = 'campus-menu';
	...
	$nav_output = sprintf( '<div id="sidebar-nav"><ul class="superfish menu sf-js-enabled"><li class="menu-item" id="explore-apu-home"><a href="'. get_permalink(21) .'">'. get_the_title(21) .'</a></li>%2$s%1$s%3$s</ul></div>', $nav, genesis_structural_wrap( 'sidebar-nav', '<div class="wrap">', 0 ), genesis_structural_wrap( 'sidebar-nav', '</div><!-- end .wrap -->', 0 ) );
