...
// Template Name: Outdoor Education
...
function template_load_the_scripts(){
	template_load_background('outdoor_education');
}
...
function template_load_mobile(){
	load_main_menu();
	load_page_menu('outdoor-education-menu');
	load_quick_links();
}
...
function template_do_sidebar_menu(){
	template_do_dept_sidebar_menu('outdoor-education-menu');
}
...
function template_do_sidebar(){
		template_do_spotlight_sidebar(76);
}
...
function template_after_post_content(){
	template_home_after_post_content(1754, 76);
}

//Add in the degree header tab
add_action('genesis_before_content_sidebar_wrap', 'section_do_title', 20);
function section_do_title(){
	?> <div class="tab-title"><h1><a href="<?php echo get_permalink(1754); ?>"><?php echo get_the_title(1754); ?></a></h1></div><?php
}
