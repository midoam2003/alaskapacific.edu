...
// Template Name: Human Services, A.A.
...
function template_load_the_scripts(){
	template_load_background('human_services_aa');
}
...
function template_load_mobile(){
	load_main_menu();
	load_page_menu('human-services-aa-menu');
	load_quick_links();
}
...
function template_do_sidebar_menu(){
	template_do_dept_sidebar_menu('human-services-aa-menu');
}
...
function template_after_post_content(){
	template_home_after_post_content(2057, 66);
}

//Add in the degree header tab
add_action('genesis_before_content_sidebar_wrap', 'section_do_title', 20);
function section_do_title(){
	?> <div class="tab-title"><h1><a href="<?php echo get_permalink(2057); ?>"><?php echo get_the_title(2057); ?></a></h1></div><?php
}
