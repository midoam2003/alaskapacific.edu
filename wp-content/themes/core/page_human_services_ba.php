...
// Template Name: Human Services, B.A.
...
function template_load_the_scripts(){
	template_load_background('human_services_ba');
}
...
function template_load_mobile(){
	load_main_menu();
	load_page_menu('human-services-ba-menu');
	load_quick_links();
}
...
function template_do_sidebar_menu(){
	template_do_dept_sidebar_menu('human-services-ba-menu');
}
...
function template_do_sidebar(){
		template_do_spotlight_sidebar(66);
}
...
function template_after_post_content(){
	template_home_after_post_content(2072, 66);
}

//Add in the degree header tab
add_action('genesis_before_content_sidebar_wrap', 'section_do_title', 20);
function section_do_title(){
	?> <div class="tab-title"><h1><a href="<?php echo get_permalink(2072); ?>"><?php echo get_the_title(2072); ?></a></h1></div><?php
}
