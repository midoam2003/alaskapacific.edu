...
class Ai1ecFacebookConnectorPlugin extends Ai1ec_Connector_Plugin {
	...
	public function __construct( Ai1ec_Registry_Object $registry ) {
		...
		$this->persistance_context = $factory->create_persistence_context( self::KEY_FOR_PERSISTANCE );
		// Install CRON
		$this->install_cron();
	}
	 ...
	 */
	private function get_current_facebook_user_from_cache() {
		try{
	...
	public function render_export_box() {
		...
		if( $event !== NULL && $facebook_status === Ai1ecFacebookConnectorPlugin::FB_IMPORTED_EVENT ) {
			return;
		}
		...
		$checked = '';
		if( $event !== NULL && $facebook_status === Ai1ecFacebookConnectorPlugin::FB_EXPORTED_EVENT ) {
			$checked = 'checked';
		}
		$link = '';
		$modal_html = '';
		...
		$tokens = array();
		$refresh_button = '';
		if( $event !== NULL && $facebook_eid && ( int ) $facebook_eid !== 0 ) {
			$link_label = __( "View associated Facebook event", AI1EC_PLUGIN_NAME );
			$link = '<div id="ai1ec-facebook-linked-event">' .
				'<a href="https://www.facebook.com/events/' . $facebook_eid .
				'" target="_blank">' . $link_label .
			...
			$twitter_bootstrap_modal = $this->_registry->get(
				'html.element.legacy.bootstrap.modal',
				 __(
				 	 "Would you like to delete the associated Facebook event or keep it? If you choose to keep it and later you export this event again, a new one will be created.",
				 	AI1EC_PLUGIN_NAME
				)
			);
			...
			$current_user = $this->get_current_facebook_user_from_cache();
			if( ! empty( $pages ) ) {
				$pages_html = Ai1ec_Facebook_Factory::create_export_to_pages_html(
					$pages,
					$current_user
				);
			}
		...
		$html = <<<HTML
<div id="ai1ec-facebook-publish" class="misc-pub-section timely">
	<label for="$name">
		<input type="checkbox" $checked name="$name" id="$name" value="1">
		<i class="ai1ec-fa ai1ec-fa-facebook-square ai1ec-fa-lg"></i>
		$label
	</label>
	$link
	<div id="ai1ec-fb-collapse" class="ai1ec-collapse">
		$pages_html
		$refresh_button
	</div>
</div>
