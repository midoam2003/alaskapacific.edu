...
class Ai1ec_Facebook_Graph_Object_Collection extends Ai1ec_Base {
	// Identifyes users.
	const FB_USER     = 'user';
	// Identifies Pages
	const FB_PAGE     = 'page';
	 ...
	 * @var int
	 */
	private $_enable_comments;

	/**
	 *
	 * @var int
	 */
	private $_display_map;
	...
	/**
	 * @param Ai1ec_Facebook_Proxy $_facebook
	 */
	public function set_facebook( Ai1ec_Facebook_Proxy $_facebook ) {
	...
	/**
	 * @param bool $_enable_comments
	 */
	public function set_enable_comments( $_enable_comments ) {
		$this->_enable_comments = $_enable_comments;
	}

	/**
	 * @param bool $_display_map
	 */
	public function set_display_map( $_display_map ) {
		$this->_display_map = $_display_map;
	}

	public function __construct( Ai1ec_Registry_Object $registry, array $ids ) {
	 ...
	 *
	 * @param boolean $subscribe if TRUE the user is subscribed, if fals it is unsubscribed.
	 *
	...
	public function update_subscription_status( $subscribe = TRUE, $remove_events = FALSE ) {
		...
		// Set the category and tag. The default for the category and the tag is an empty string.
		$category   = isset( $this->_category ) ? $this->_category : '';
		$tag        = isset( $this->_tag ) ? $this->_tag : '';
		$enabled    = ( $this->_enable_comments ) ? 1 : 0;
		$mapping    = ( $this->_display_map ) ? 1 : 0;
		$user       = array(
			'category'            => $this->_category,
			'comments_enabled'    => $enabled,
			'map_display_enabled' => $mapping,
			'tag'                 => $this->_tag,
			'subscribed'          => $status,
		);
	...
	private function get_users_already_in_the_db( array $users ) {
		...
		$query = $wpdb->prepare(
			'SELECT user_id FROM ' . $table_name .
				' WHERE user_id IN ' . $generated_escape_sequence,
			$array_of_ids
		);
	 ...
	 *
	 * @return array the events present in the user_events table that start after the $timestamp
	 */
	private function get_events_from_user_events_table_for_collection( $timestamp ) {
		...
		$query               = $wpdb->prepare(
			'SELECT DISTINCT eid FROM ' . $table_user_events .
			' WHERE user_id IN ' . $generated_statement .
			' AND start > ' . $timestamp,
			$this->_ids
		);
	...
	private function handle_saving_in_user_events_table( $user_id, $eid, $timestamp ) {
		...
		$query      = $wpdb->prepare(
			'SELECT \'exist\' FROM ' . $table_name .
				' WHERE user_id = %s AND eid = %s',
			$user_id,
			$eid
		);
	...
	private function save_event( array $event, $user_timezone ) {
		...
		// now save data in post-meta
		$facebook_meta = $this->_registry->get( 'facebook.meta' );
		$facebook_meta->save_facebook_eid( $new_post_id, $event['eid'] );
		$facebook_meta->save_facebook_status( $new_post_id, Ai1ecFacebookConnectorPlugin::FB_IMPORTED_EVENT );
		$facebook_meta->save_facebook_user( $new_post_id, $event['facebook_user'] );
		// Throw an Exception if saving post failed for some reason.
		if ( $new_post_id === 0 ) {
			throw new Exception(
				__( 'Could not save a new event during Facebook import.', AI1EC_PLUGIN_NAME )
			);
		}
	}
	/**
	...
	private function convert_facebook_event_to_ai1ec_event( array $facebook_event, $user_timezone, $post_id = NULL ) {
		...
		if ( isset( $facebook_event['start_time'] ) ) {
			$event->set( 
				'start',
				Ai1ec_Facebook_Event::convert_facebook_time_to_gmt_timestamp( $facebook_event['start_time'], $user_timezone )
			);
		}
		if (
			...
			$event->set( 
				'end',
				Ai1ec_Facebook_Event::convert_facebook_time_to_gmt_timestamp( $facebook_event['end_time'], $user_timezone )
			);
		}
		if ( $event->get( 'start' )->format_to_gmt() <= 0 ) {
			throw new Ai1ec_Invalid_Argument(
				'Event start time is undefined/invalid'
			);
		}
		if ( $event->get( 'end' )->format_to_gmt() <= 0 ) { // make it an instantaneous event
			$event->set( 'end', $event->get( 'start' ) + 1800 );
			$event->set( 'instant_event', 1 );
		}
		...
		$event->set( 
			'venue',
			Ai1ec_Facebook_Data_Converter::return_empty_or_value_if_set( $facebook_event, 'location' )
		);
		$event->set(
			'address',
			Ai1ec_Facebook_Data_Converter::return_empty_or_value_if_set( $facebook_event['venue'], 'street' )
		);
		$event->set(
			'city',
			Ai1ec_Facebook_Data_Converter::return_empty_or_value_if_set( $facebook_event['venue'], 'city' )
		);
		$event->set(
			'province',
			Ai1ec_Facebook_Data_Converter::return_empty_or_value_if_set( $facebook_event['venue'], 'state' )
		);
		$event->set( 
			'postal_code',
			Ai1ec_Facebook_Data_Converter::return_empty_or_value_if_set( $facebook_event['venue'], 'zip' )
		);
		$event->set(
			'country',
			Ai1ec_Facebook_Data_Converter::return_empty_or_value_if_set( $facebook_event['venue'], 'country' )
		);
		...
		if ( ! $event->get( 'address' ) ) {
			if ( false !== strpos( $event->get( 'venue' ), ',' ) ) {
				$event->set( 'address', $event->get( 'venue' ) );
				$event->set ( 'venue', '' );
			}
		}
		else {
			...
			$event->set( 'address', implode( ', ', array_filter( $address ) ) );
		}

		// We show the map if we have a valid address or we have coordinates. This is arbitrary.
		$event->set( 'show_map', $coordinates_set || $event->get( 'address' ) );

		$data = Ai1ec_Facebook_Factory::get_facebook_graph_object(
				$facebook_event['facebook_user']
			)->get_category_and_tag();
		if ( ! $data['map_display_enabled'] ) {
			$event->set( 'show_map', false );
		}
		$event->set( 'show_coordinates', $coordinates_set ? 1 : 0 );
		if ( $coordinates_set ) {
			$event->set( 'longitude', $facebook_event['venue']['longitude'] );
			$event->set( 'latitude', $facebook_event['venue']['latitude'] );
		}
		...
		$event->set( 'categories', (array) $data['category'] );
		$event->set( 'tags', (array) $data['tag'] );
		...
		if ( $post_id === NULL ) {
			// if it's a new event set the correct status
			$settings            = $this->_registry->get( 'model.settings' );
			$status              = $settings->get( 'facebook_import_draft' )
				? 'draft'
				: 'publish';
		...
		// Else we are updating an existing event, so return a data array.
		else {
			$post['ID'] = $post_id;
	...
	private function update_event( array $event, $user_timezone, $post_id ) {
		...
		$event_to_check = $this->_registry->get( 'model.event',  $post_id );
		$facebook_meta = $this->_registry->get( 'facebook.meta' );
		$meta = $facebook_meta->get_facebook_meta( $event_to_check->get( 'post_id' ) );
		$facebook_status = $meta['facebook_status'];
		// IF the event is not an imported event, do not touch it
		if ( $facebook_status !== Ai1ecFacebookConnectorPlugin::FB_IMPORTED_EVENT ) {
			return;
		}
		...
		if ( $return === 0 ) {
			throw new Exception(
				__( 'Could not update an event post during Facebook import.', AI1EC_PLUGIN_NAME )
			);
		}
		// Update categories & tags.
		wp_set_post_terms( $post_id, $ai1ec_event->get( 'categories' ), 'events_categories' );
		wp_set_post_terms( $post_id, $ai1ec_event->get( 'tags' ), 'events_tags' );
